...
def main():
    ...
    mode = "収束" if nproj <= 3 else ("標準" if nproj <= 6 else "発散")
    ...
    print(f"nodes: {len(nodes)} (知識 {k} / 台帳 {pj} / 文脈 {ctx} / 🖥ライブ {len(live)})")
    print(f"links: {len(links)} (wiki {lk['wiki']} / 垂直 {lk['vertical']} / "
          f"ライブ {lk['live']} / ライブハブ {lk['live-hub']})")
    print(f"影響半径: 直近{RECENT_DAYS}日の起点 {len(recent)}ノート → 到達 {reach}/{k}")
    for t in thaw:
        via = "・".join(f"{v['label']}(d{v['dist']})" for v in t["via"])
        print(f"🌱 解凍候補: {t['project']} (score {t['score']}, "
              f"最終 {t['last_touch']}) ← {via}")
    if not thaw:
        print("🌱 解凍候補: なし（凍結台帳と最近の作業に近接なし）")
    fm_ = focus_meter
    print(f"🧭 思考モード: {fm_['mode']}（直近{RECENT_DAYS}日に触れた案件 {fm_['recent_projects']}件"
          f" / 窓 {fm_['live_connected']}接続・{fm_['live_unconnected']}未帰属）")
    if in_head:
        top = "・".join(f"{l}({b}字)" for l, b, _ in in_head[:5])
        print(f"📄 記録が薄い稼働案件 {len(in_head)}件（頭の中に偏りがち）: {top}")
    print(f"dangling wikilinks: {sum(dangling.values())}箇所 (→ out/dangling.txt), "
          f"曖昧解決(同dir/同vault優先): {rstats['ambiguous']}")
    print(f"out: {OUT_DIR / 'thought-net.html'} (単一ファイル)")
    return 0
